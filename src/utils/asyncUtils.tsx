...
export interface ReducerUtilsTypes {
  init: () => {
    data: null;
    loading: false;
    error: false;
...
interface ResponseGeneratorType {
  config?: any;
  data?: any;
  headers?: any;
  request?: any;
 ...
 */
export const createRequestSaga = (type: string, request: any) => {
  ...
  return function* (action: { type: string; payload: any }) {
    ...
    try {
      const response: ResponseGeneratorType = yield call(
        request,
        action.payload
      );
      ...
      const payload = target ? { msg: "Error", target } : { msg: "Error" };
